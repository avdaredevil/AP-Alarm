...
function KeyChecks($num, $tot) {
    while ($Host.UI.RawUI.KeyAvailable) {
        ...
        If ($tot -and (KeyPressed "~~Space~~","~~Enter~~","-" $Store)) {Write-AP ">!Sleep cycle [$num / $tot] for 3 secs"}
    }
}
function Quit {
...
if (!(test-path $AlarmScript -type leaf)) {Write-AP "!Alarm script is missing, will need to use videos"}
